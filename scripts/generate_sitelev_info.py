...
def process_coverage(start_pos,end_pos,cutoff,filename):
    ...
    #                 pos_coverage[chr_name][pos]+=predict_label
    #                 coverage[chr_name][pos]+=1
    return pos_coverage,coverage,predict_scores,strands
def aggregate_and_generate_output(list_of_pos_coverage,list_of_coverage,list_of_predict_scores,list_of_strands,outputname,cov_cutoff,ratio_cutoff,mod_cov_cutoff):
    all_pos_coverage = list_of_pos_coverage[0]
    all_coverage = list_of_coverage[0]
    all_predict_scores=list_of_predict_scores[0]
    all_strands=list_of_strands[0]
    if len(list_of_pos_coverage) != 1:
        for pos_coverage,coverage,predict_scores,strands in zip(list_of_pos_coverage[1:],list_of_coverage[1:],list_of_predict_scores[1:],list_of_strands[1:]):
    ...
    with open(outputname,'w') as output:
        for chr_name in sorted(all_coverage.keys()):
            for pos in sorted(all_coverage[chr_name].keys()):
                ...
                output.write(f'{chr_name}\t{pos}\t{all_pos_coverage[chr_name][pos]}\t{all_coverage[chr_name][pos]}\t{ratio:.3f}\t{avg_predict_score:.3f}\t{all_strands[chr_name][pos]}\n')
...
if __name__ == "__main__":
    ...
    list_of_pos_coverage,list_of_coverage,list_of_predict_scores,list_of_strands = [],[],[],[]
    for future in futures:
        pos_coverage,coverage,predict_scores,strands = future.get()
        list_of_pos_coverage.append(pos_coverage)
        list_of_coverage.append(coverage)
        list_of_predict_scores.append(predict_scores)
        list_of_strands.append(strands)
    pool.close()
    pool.join()
    
    aggregate_and_generate_output(list_of_pos_coverage,list_of_coverage,list_of_predict_scores,list_of_strands,outputname,cov_cutoff,ratio_cutoff,mod_cov_cutoff)
