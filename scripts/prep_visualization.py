...
def get_line_marker(mole_path,threads):
    ...
    return line_marker
def mole_info_to_bam_worker(mole_path,reference_type,reference_id_dict,reference_path,mole_output_dir,i,thres,marker):
...
#                 seg.tags = [('SC',get_color(np.mean(score_list)),'Z'),('ML',array.array('f',score_list),'f')]
            ...
            output_handle.write(seg)
            output_handle.close()
def mole_info_to_bam(mole_path,reference_id_dict,reference_path,mole_output_dir,reference_type,threads):
...
#         sitelevel_df[['rname','start','end','model_score']].to_csv(f'{output_dir}/model_score.bedgraph',sep='\t',header=None,index=None)
    ...
    if args.mole_level_bed is not None:
        ...
        mole_info_to_bam(mole_path,reference_id_dict,reference_path,output_dir,args.reference_type,args.threads)
        print('DONE in '+str(time.time()-st_time)+' seconds',flush=True)
main()